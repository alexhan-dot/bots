...
| `Staff` | 디스코드 ID ↔ 스케줄 이름. 디스코드 이름이 스케줄 이름과 같으면 첫 `/shot` 때 자동 연결, 아니면 봇이 ID 로 줄을 만들고 매니저가 Name 칸을 채움 | 봇 + 매니저 |
...
| `/shot` `/myshifts` | 플레이어용 (역할 불필요) | `/shot image:<스크린샷>` |
...
**플레이어 스크린샷** (`#shift-reports`) — 시프트 시작·끝에 `/shot image:` → 지금 시간대 스케줄에서 캐릭터를 찾고 AI 가 레벨·EXP %·아데나를 읽어 **바로 기록** (읽은 것만, 틀리면 Fix · 시작/끝 바꾸기). 끝에서 EXP 획득 % = (끝 레벨−시작 레벨)×100 + 끝% − 시작%, 아데나 획득 = 끝 − 시작. `/myshifts` 내 3일 스케줄. 플레이어는 Manager 역할 없이 이 명령만 사용 가능.
